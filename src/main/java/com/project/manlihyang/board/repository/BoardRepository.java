...
public interface BoardRepository {
    ...
    //게시물 생성
    @Insert("INSERT INTO board (bsn, writer_id, title, content, img_url, img_name, bg_img_url, report_cnt, group_id, group_seq, group_depth) VALUES(#{bsn}, #{writer_id}, #{title}, #{content}, #{img_url}, #{img_name}, #{bg_img_url}, ${report_cnt}, #{group_id}, ${group_seq}, ${group_depth})")
    ...
    void BoardDeleteRepo(String bsn);
    ...
    void BoardReportRepo(String bsn, int report_cnt);

    //5번 이상 신고된 게시물 삭제
    @Delete("Delete from board where bsn = #{bsn}")
    void BoardReportDelRepo(String bsn);

    //게시물의 댓글 조회
    @Select("SELECT * FROM board where group_id = #{bsn} && bsn != #{bsn}")
    List<Board> BoardCommentsReadRepo(String bsn);
    ...
    void BoardCommentCreateRepo(Board comment);
